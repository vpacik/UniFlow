...
void PlotFluctWithSystHistosPreliminary()
{
    SetStyle();

    TString sInDir = "./";
    TString sInDirSyst = "./syst/";
    TString sOutDir = "./plots/";

    TString sLabel = "Fv2"; TString sHistName = "rel"; TString yAxisTitle = "#it{F}(#it{v}_{2})";
    TString sHistNameSyst = "rel";
    ...
    Double_t xMin = 0.0; Double_t xMax = 4.0;
    Double_t yMin = 0.25; Double_t yMax = 0.65;
    ...
    sSpecies.push_back("Kaon"); sSpesLabels.push_back(" K^{#pm}"); colors.push_back(kGreen+3); markers.push_back(kFullTriangleDown); markerSizes.push_back(1); systs.push_back(0.04);
    sSpecies.push_back("K0s"); sSpesLabels.push_back(" K^{0}_{S}"); colors.push_back(kGreen+1); markers.push_back(kFullTriangleUp); markerSizes.push_back(1); systs.push_back(0.07);
    ...
    // sSpecies.push_back("Phi"); sSpesLabels.push_back("#phi"); colors.push_back(kMagenta); markers.push_back(kFullCross); markerSizes.push_back(1.); systs.push_back(0.08);


    TFile* fileIn = TFile::Open(Form("%s/FlowFluctCor.root",sInDir.Data()),"READ");
    ...
    for(Int_t iMult(0); iMult < iNumMult; ++iMult) {
        ...
        if(iMult == 0) {
            leg = new TLegend(0.19,0.19,0.36,0.35);
            leg2 = new TLegend(0.29,0.19,0.47,0.35);

            text2->DrawLatexNDC(0.6,0.29,Form("ALICE Preliminary"));
            text->DrawLatexNDC(0.6,0.24,Form("Pb-Pb #sqrt{#it{s}_{NN}} = 5.02 TeV"));
            text->DrawLatexNDC(0.6,0.19,Form("|#it{#eta}| < 0.8"));
            text->DrawLatexNDC(0.19,0.37,sCent[iMult].Data());
        } else {
            leg = new TLegend(0.67,0.66,0.84,0.82);
            leg2 = new TLegend(0.77,0.66,0.93,0.82);
