...
void ScaleVn()
{
    TFile* fileIn = TFile::Open("/Users/vpacik/Codes/ALICE/Flow/uniFlow/thesis/PbPb/cums/0010/v2/graphs_PbPb_v22_v24.root","READ");
    if(!fileIn) { printf("File in not open!\n"); return; }

    TFile* fileOut = TFile::Open("/Users/vpacik/Codes/ALICE/Flow/uniFlow/thesis/PbPb/cums/0010/v2/graphs_PbPb_v22_v24_scaled.root","RECREATE");
    ...
    for(Int_t s(0); s < iSpecies; ++s)
    {
        for(Int_t h(0); h < iHistos; ++h)
        {
            for(Int_t c(0); c < iCent; ++c)
            {
                ...
                TString sNameNCQ = Form("%s_%s_NCQ_cent%s", sSpecies[s].Data(), sHistoName[h].Data(), sCent[c].Data());
                TString sNameKEt = Form("%s_%s_KEt_cent%s", sSpecies[s].Data(), sHistoName[h].Data(), sCent[c].Data());

                TGraphErrors* gr = (TGraphErrors*) fileIn->Get(sName.Data());

                // ncq-like scaling
                TGraphErrors* scaled = Scale(gr,dScale[s],dScale[s]);
                scaled->SetName(sNameNCQ.Data());
