...
void Plot_PbPb_perCent_All_Fv2_theory()
{
    // === Preparing canvas =================

    TString sInputFileNameModel = "PbPb/models/graphs_PbPb_Fv2_AMPT.root";
    ...
    TString sOutPlotName = "PbPb_perCent_All_Fv2_theory.pdf";
    ...
    TLegend* legAMPT = new TLegend(0.05,0.69,0.8,0.96);
    legAMPT->SetHeader("AMPT");
    SetLegendStyle(legAMPT);

    TLegend* legTRENTo = new TLegend(0.28,0.69,1.,0.96);
    ...
    for(Int_t p(0); p < iNumPads; ++p) {
        ...
        if(p > 0) {
            ...
            // plotting models
            TGraphErrors* grModPion = (TGraphErrors*) fileInModel->Get(Form("Pion_Fv2_%s",sPadName.Data()));
            if(!grModPion) { printf("Pion model not found!\n"); fileInModel->ls(); return; }

            TGraphErrors* grModKaon = (TGraphErrors*) fileInModel->Get(Form("kaon_Fv2_%s",sPadName.Data()));
            if(!grModKaon) { printf("Kaon model not found!\n"); fileInModel->ls(); return; }

            TGraphErrors* grModProton = (TGraphErrors*) fileInModel->Get(Form("proton_Fv2_%s",sPadName.Data()));
            ...
            // plotting TRENTO
            TGraphErrors* grModPionTRENTo = (TGraphErrors*) fileInTRENTo->Get(Form("Pion_Fv2_%s",sPadName.Data()));
            if(!grModPionTRENTo) { printf("Pion model not found!\n"); fileInTRENTo->ls(); return; }

            TGraphErrors* grModKaonTRENTo = (TGraphErrors*) fileInTRENTo->Get(Form("kaon_Fv2_%s",sPadName.Data()));
            if(!grModKaonTRENTo) { printf("Kaon model not found!\n"); fileInTRENTo->ls(); return; }

            TGraphErrors* grModProtonTRENTo = (TGraphErrors*) fileInTRENTo->Get(Form("proton_Fv2_%s",sPadName.Data()));
