...
void Plot_PbPb_perCent_All_v22gap08_theory()
{
    // === Preparing canvas =================
    ...
    TString sOutPlotName = "PbPb_perCent_All_v22gap08_theory.pdf";
    ...
    TGaxis* gaY = new TGaxis(dXmin, 0.0, dXmin, 0.40, 0.0,0.40);
    ...
    for(Int_t p(0); p < iNumPads; ++p) {
        ...
        if(p > 0) {
            ...
            // plotting models
            TGraphErrors* grModPion = (TGraphErrors*) fileInModel->Get(Form("Pion_v22_%s",sPadName.Data()));
            if(!grModPion) { printf("Pion model not found!\n"); fileInModel->ls(); return; }

            TGraphErrors* grModKaon = (TGraphErrors*) fileInModel->Get(Form("kaon_v22_%s",sPadName.Data()));
            if(!grModKaon) { printf("Kaon model not found!\n"); fileInModel->ls(); return; }

            TGraphErrors* grModProton = (TGraphErrors*) fileInModel->Get(Form("proton_v22_%s",sPadName.Data()));
            ...
            // plotting TRENTO
            TGraphErrors* grModPionTRENTo = (TGraphErrors*) fileInTRENTo->Get(Form("Pion_v22_%s",sPadName.Data()));
            if(!grModPionTRENTo) { printf("Pion model not found!\n"); fileInTRENTo->ls(); return; }

            TGraphErrors* grModKaonTRENTo = (TGraphErrors*) fileInTRENTo->Get(Form("kaon_v22_%s",sPadName.Data()));
            if(!grModKaonTRENTo) { printf("Kaon model not found!\n"); fileInTRENTo->ls(); return; }

            TGraphErrors* grModProtonTRENTo = (TGraphErrors*) fileInTRENTo->Get(Form("proton_v22_%s",sPadName.Data()));
