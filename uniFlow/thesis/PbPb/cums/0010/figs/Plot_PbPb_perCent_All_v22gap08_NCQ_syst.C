...
void Plot_PbPb_perCent_All_v22gap08_NCQ_syst()
{
    // === Preparing canvas =================

    TString sInputFileName = "PbPb/cums/0010/v2/graphs_PbPb_v22_v24_scaled.root";
    TString sOutPlotName = "PbPb_perCent_All_v22gap08_NCQ_syst.pdf";
    ...
    gaX->SetTitle("#it{p}_{T}/#it{n}_{q} (GeV/#it{c})");
    TGaxis* gaY = new TGaxis(dXmin, 0.0, dXmin, 0.12, 0.0,0.12);
    gaY->SetTitle("#it{v}_{2}{2,|#Delta#eta| > 0.8}/#it{n}_{q}");
    TString sCorrName = "v2{2,gap08}_NCQ";
    ...
    for(Int_t p(0); p < iNumPads; ++p) {
        ...
        for(Int_t h(0); h < iNumHistos; ++h) {
            ...
            // SetMarkerPID(grSyst,kSpecies(h));
            // grSyst->SetLineColor(colors[i]);
