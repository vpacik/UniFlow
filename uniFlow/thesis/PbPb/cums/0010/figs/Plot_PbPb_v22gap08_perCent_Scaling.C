...
void Plot_PbPb_v22gap08_perCent_Scaling()
{
    // === Preparing canNCQvas =================
    ...
    TGaxis* gaY = new TGaxis(dXmin, 0.0, dXmin, 0.13, 0.0,0.13);
    gaY->SetTitle("v_{2}{2,|#Delta#eta|>0.8}/n_{q}");
    ...
    for(Int_t i(0); i < iNumCent; ++i) {
        ...
        if(i < 3) leg1->AddEntry(gr,sLabels[i].Data(),"pl");
    ...
    leg1->Draw();
    leg2->Draw();
    canNCQ->SaveAs("./plots/PbPb_v22gap08_perCent_NCQ.pdf","pdf");

    canKEt->cd(1);
    leg1->Draw();
    leg2->Draw();
    canKEt->SaveAs("./plots/PbPb_v22gap08_perCent_KEt.pdf","pdf");
}
