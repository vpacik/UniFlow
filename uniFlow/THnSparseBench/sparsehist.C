...
void sparsehist() {
   ...
   for (int h = 0; h < TTimeHists::kNumHist; ++h)
      for (int t = 0; t < TTimeHists::kNumTime; ++t) {
         ...
         htime[h][t] = new TH2F(name, title, 6, 0.5, 6.5, 10, 105, 205);
      }

   TH2F* hsparse_mem = new TH2F("hsparse_mem", "Fractional memory usage;dim;bins;fraction of memory used", 6, 0.5, 6.5, 10, 105, 205);
   TH2F* hsparse_bins = new TH2F("hsparse_bins", "Fractional number of used bins;dim;bins;fraction of filled bins", 6, 0.5, 6.5, 10, 105, 205);

   // TTimeHists::SetDebug(2);
   Double_t max = -1.;
   for (Int_t dim = 1; dim < 7; ++dim) {
      printf("Processing dimension %d", dim);
      for (Int_t bins = 100; bins <= 200; bins += 10) {
         TTimeHists timer(dim, bins, /*num*/ 100000);
         timer.Run();
         for (int h = 0; h < TTimeHists::kNumHist; ++h)
            for (int t = 0; t < TTimeHists::kNumTime; ++t) {
